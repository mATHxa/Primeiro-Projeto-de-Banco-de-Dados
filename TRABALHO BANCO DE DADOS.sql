...
telefoneEditora varchar(20) not null,
emailEditora varchar(50) not null, 
enderecoEditora varchar(100) not null
);

create table Estoque(
Livro_id int,
foreign key(Livro_id) references Livro(idLivro),
quantidade int not null
);


alter table Livro
change ano ano year not null;
select * from Estoque;
show tables;

-------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into Cliente (idCliente, nomeCliente, telefoneCliente, emailCliente, enderecoCliente) values
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into Editora (idEditora, nomeEditora, telefoneEditora, emailEditora, enderecoEditora) values
...
insert into Livro (idLivro, id_Editora, titulo, autor, ano, ISBN, preco) values
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into Pedido (idPedido, id_Cliente, dataPedido, valorPedido) values
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into ItemPedido (id_Pedido, id_Livro, quantidadePedido, valorItemPedido) values
     (1, 4, 1, 42.90),
     (2, 8, 1, 119.90),
     (3, 5, 2, 111.80),
     (3, 2, 1, 35.90),
...
select EditoraApelido.nomeEditora, LivroApelido.Titulo
from Editora EditoraApelido
join Livro LivroApelido on EditoraApelido.idEditora = LivroApelido.id_Editora
order by EditoraApelido.nomeEditora desc; 


select Editora.nomeEditora, avg(Livro.preco) as mediaPreco
from Editora
join Livro on Editora.idEditora = Livro.idLivro
group by Editora.nomeEditora;


select Cliente.nomeCliente, sum(itemPedido.quantidadePedido) as QuantidadePedido
from Cliente
join Pedido on Cliente.idCliente = Pedido.id_Cliente
join ItemPedido on Pedido.idPedido = ItemPedido.id_Pedido
